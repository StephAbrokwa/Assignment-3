...
upd_correct_letters <- function(guess) {
  correct_letters[strsplit(secret_word, "")[[1]] == guess] <- guess
}

# Function to display the progress 
display_progress <- function() {
  cat(paste0(correct_letters, colapse = " "), "\n")
}
...
while (wrong_guesses < max_attempts) {
  ...
  if (nchar(user_input) !=1 || !is_letter(user_inptut)) {
  ...
  if (tolower(user_input) == "guess") {
    ...
    if (tolower(guessed_word) == answer) {
  ...
  if (grepl(user_input, answer, ignore.case = TRUE)) {
    cat("Correct guess!")
    upd_correct_letters(user_input)
    display_progress()
